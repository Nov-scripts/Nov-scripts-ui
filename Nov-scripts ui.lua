local Nexus = {}
Nexus.\_\_index = Nexus
\_G.Primary = Color3.fromRGB(0,191,255)
...
local function safe\_isfolder(p) return isfolder and isfolder(p) end
local function safe\_makefolder(p) if makefolder then makefolder(p) end end
local function safe\_isfile(p) return isfile and isfile(p) end
local function safe\_readfile(p) if readfile then return readfile(p) end end
local function safe\_writefile(p, d) if writefile then writefile(p, d) end end
local function safe\_delfolder(p) if delfolder then delfolder(p) end end
...
local old = CoreGui\:FindFirstChild("NexusUI\_Full")
if old then pcall(function() old\:Destroy() end) end
local screen = Instance.new("ScreenGui", CoreGui)
screen.Name = "NexusUI\_Full"
screen.ResetOnSpawn = false
local container = Instance.new("Frame", screen)
container.Name = "Container"
container.Size = UDim2.new(0, 104.5, 0, 52)
...
midSection.Position = UDim2.new(0.444,0,0.5,0)
...
lib.\_config = { \_flags = {}, *keybinds = {}, *library = {} }
if safe\_isfolder("Nexus") then
if safe\_isfile("Nexus/config*"..tostring(game.GameId)..".json") then
local ok, data = pcall(function() return HttpService\:JSONDecode(safe\_readfile("Nexus/config*"..tostring(game.GameId)..".json")) end)
if ok and type(data) == "table" then lib.*config = data end
end
else
safe\_makefolder("Nexus")
end
function lib\:SaveConfig()
pcall(function()
safe\_writefile("Nexus/config*"..tostring(game.GameId)..".json", HttpService\:JSONEncode(self.\_config))
end)
end
...
moduleAPI.\_size = 0
moduleAPI.*flag = settings.flag or ("module*" .. tostring(math.random(1,99999)))
...
function moduleAPI\:change\_state(state)
self.\_state = state
if state then
...
moduleAPI\:change\_state(not moduleAPI.\_state)
end)
...
if conn then conn\:Disconnect(); choosing = false end
moduleAPI\:connect\_keybind()
lib\:SaveConfig()
end)
end
end)
function moduleAPI\:connect\_keybind()
...
table.insert(lib.*tabs, tabAPI)
return tabAPI
end
return lib
end
return setmetatable({}, { \_\_call = function(*, ...) return Nexus end })
